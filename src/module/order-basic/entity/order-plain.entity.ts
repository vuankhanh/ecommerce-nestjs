...
import { ORDER_FROM_LABEL, ORDER_STATUS_LABEL } from "src/constant/order.constant";
...
export class OrderItemPlainEntity implements Omit<OrderItemEntity, 'productName'> {
...
export class OrderPlainEntity implements Omit<IOrderDetailPopulated, 'orderFrom' | 'orderItems' | 'status' | 'paymentMethod'> {
  orderCode: string;
  ...
  constructor(order: IOrderDetailPopulated, lang: TLanguage) {
    ...
    this.orderItems = OrderPlainEntity.getOrderItemsPlain(order.orderItems, lang);
    ...
    this.paymentMethod = OrderPlainEntity.getPaymentMethodPlain(order.paymentMethod, lang);
    ...
    this.delivery['addressDetail'] = DeliveryEntity.generateAddressDetail(this.delivery.address);
  }

  static getOrderItemsPlain(orderItems: OrderItemEntity[], lang: TLanguage): OrderItemPlainEntity[] {
  ...
  static getOrderStatusPlain(orderStatus: TOrderStatus, lang: TLanguage): string {
    return ORDER_STATUS_LABEL[orderStatus]?.[lang] || orderStatus;
  }

  static getPaymentMethodPlain(orderPaymentMethod: TPaymentMethod, lang: TLanguage): string {
    return PAYMENT_METHOD_LABEL[orderPaymentMethod]?.[lang] || orderPaymentMethod;
  }
}
