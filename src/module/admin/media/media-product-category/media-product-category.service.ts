...
export class MediaProductCategoryService implements IBasicAdminService<Album> {
  ...
  async checkExistProductCategoryAlbum(
  ...
  async getAll(filterQuery: FilterQuery<Album>, page: number, size: number) {
    filterQuery.purposeOfMedia = PurposeOfMedia.PRODUCT_CATEGORY;
    const countTotal =
      await this.productCategoryAlbumModel.countDocuments(filterQuery);
  ...
  async getMainProductCategory(
    ...
    filterQuery.purposeOfMedia = PurposeOfMedia.PRODUCT_CATEGORY;
    return await this.productCategoryAlbumModel
      .findOne(filterQuery)
  ...
  async filterMediaItems(
    ...
    return this.productCategoryAlbumModel
      .aggregate([
        { $match: filterQuery },
        {
          $project: {
