...
export class AccountIdGuard implements CanActivate {
  ...
  async canActivate(
    ...
    if (!payload || !payload.email) {
      throw new CustomBadRequestException('Payload không hợp lệ hoặc không chứa email');
    }

    const { email } = payload;
    const accountId: string = await this.accountService.getIdAccountByEmail(email);
