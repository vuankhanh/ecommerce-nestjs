...
import {
  Product_Category,
  ProductCategoryDetailPopulatedDocument,
  ProductCategoryDocument,
  ProductCategoryPopulatedDocument,
...
{
  constructor(
    @InjectModel(Product_Category.name)
    private readonly productCategoryModel: Model<Product_Category>,
  ) {}

  async create(data: Product_Category): Promise<ProductCategoryDocument> {
    // Validate circular reference nếu có parentId
    if (data.parentId) {
      const isValid = await this.validateParentId(
        null,
        data.parentId.toString(),
      );
      if (!isValid) {
        throw new CustomBadRequestException(
          'Đã phát hiện danh mục cha mẹ hoặc tham chiếu vòng tròn không hợp lệ',
        );
      }
    }
  ...
  async getAll(
    ...
    data: FlattenMaps<ProductCategoryPopulatedDocument>[];
    paging: IPaging;
  }> {
    filterQuery.isActive = true;
    const countTotal =
      await this.productCategoryModel.countDocuments(filterQuery);
  ...
  getRawData(): Promise<ProductCategoryDocument> {
    ...
    const [data] = await this.productCategoryModel.aggregate([
      ...
      },
      {
        $addFields: {
          ...
          'parent.name': {
            $ifNull: ['$parent.name.' + lang, '$parent.name.vi'],
          },
          'parent.description': {
            $ifNull: ['$parent.description.' + lang, '$parent.description.vi'],
          },
  ...
  async replace(
    ...
    return await this.productCategoryModel.findOneAndReplace(
      filterQuery,
      data,
      { new: true },
    );
  ...
  async modify(
    ...
    if (data.parentId) {
      const currentCategory =
        await this.productCategoryModel.findOne(filterQuery);
      if (!currentCategory) {
      ...
      if (!isValid) {
        throw new CustomBadRequestException(
          'Không thể cập nhật danh mục với parentId không hợp lệ hoặc tham chiếu vòng tròn',
        );
      }
    }
    try {
  ...
  private async validateParentId(
    ...
    const parentExists = await this.productCategoryModel.exists({
      _id: parentId,
    });
    if (!parentExists) {
      return false;
