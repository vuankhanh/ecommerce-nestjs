...
const fileFilter = (req: Request, file: Express.Multer.File, cb) => {
  ...
  const match = req.url.includes(urlCustomerUpload)
    ? matchContactFile
    : matchMediaFile;
...
export const imageTypes: Record<`${ImageEnum}`, IFileType> = {
  webp: {
    type: 'image/webp',
    extension: 'webp',
  },
...
export const videoTypes: Record<`${VideoEnum}`, IFileType> = {
  mp4: {
    type: 'video/mp4',
    extension: 'mp4',
  },
